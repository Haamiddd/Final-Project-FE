...
const BadmintonBooking = () => {
  ...
  useEffect(() => {
    ...
    const todaysBookings = existingBookings.filter(booking => booking.sport === 'Badminton' && booking.date === selectedDate.toLocaleDateString());
  ...
  const handleBooking = (time) => {
    ...
    alert(`Badminton booked for ${time} on ${selectedDate.toLocaleDateString()}`);
  };

  const isSlotBooked = (time) => bookedSlots.includes(time);

  return (
    <div className="background-bad p-5">
      <Header/>
